...
public class StateMgrCS : MonoBehaviour {
    ...
    public void SetAid()
    {
        ...
        if (_playerInfoCS._currMoney >= _currSelectAid._Contract)
        {
            ...
            UseAidSys(false);
            _playerInfoCS._currUseAid = _currSelectAid;

            if (_currSelectAid._Token != 0) _playerInfoCS._clueTokenValue += _currSelectAid._Token;
            else if (_currSelectAid._Restore != 0) _playerInfoCS.setHpPoint(_currSelectAid._Restore);
            else if (_currSelectAid._Money != 0) _playerInfoCS.setMoney(_currSelectAid._Money);
            else if (_currSelectAid._Move != 0) _playerInfoCS.setActPoint(_currSelectAid._Move);
